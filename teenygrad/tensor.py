...
class Tensor:
    """Tensor object which supports back-propagation and internally represents values as numpy arrays.

    Attributes:
        name: The name of the tensor (for debugging purposes).
        value: The value of the tensor.
        grad: The current gradient of the tensor, if it has been calculated. This gradient should have
            the same shape as the value array.
    """
    name: str
    value: np.ndarray
    ...
    def __init__(
        ...
        Args:
            ...
            requires_grad: Whether gradients should be computed for the tensor (e.g., whether the tensor
                corresponds to weights or constant data).
    ...
    def sum(self) -> 'Tensor':
        """See analogous torch Tensor function."""
    ...
    def pow(self, other: float | int) -> 'Tensor':
        ...
        def _backward_fn(out: Self) -> None:
            ...
            if self.grad is not None:
                self.grad += _squash_gradient((other * self.value ** (other - 1)) * out.grad, self.grad.shape)
    ...
    def tensordot(self, other: Self, dims: int = 2) -> 'Tensor':
        """See analogous torch Tensor function."""
        def _backward_fn(out: Self) -> None:
            assert out.grad is not None
            ...
            if self.grad is not None:
                to_contract = (out.grad.ndim + other.value.ndim - self.grad.ndim) // 2
                self.grad += np.tensordot(out.grad, _transpose(other.value, to_contract), axes=to_contract)
            if other.grad is not None:
                to_contract = (out.grad.ndim + self.value.ndim - other.grad.ndim) // 2
                other.grad += np.tensordot(_transpose(self.value, self.value.ndim - to_contract), out.grad, axes=to_contract)
    ...
    def __add__(self, other: Self | ArrayLike) -> 'Tensor':
        ...
        def _backward_fn(out: Self) -> None:
            assert out.grad is not None
            if self.grad is not None:
                self.grad += _squash_gradient(out.grad, self.grad.shape)
            if _other.grad is not None:
                _other.grad += _squash_gradient(out.grad, _other.grad.shape)
    ...
    def __mul__(self, other: Self | ArrayLike) -> 'Tensor':
        ...
        def _backward_fn(out: Self) -> None:
            if self.grad is not None:
                self.grad += _squash_gradient(out.grad * _other.value, self.grad.shape)
            if _other.grad is not None:
                _other.grad += _squash_gradient(out.grad * self.value, _other.grad.shape)
    ...
    def backward(self) -> None:
        ...
        This method will compute fresh gradients for (the transitive closure of) all ancestors in the tensor's
        computation graph. Only nodes where ``requires_grad`` has been set to True will have gradients computed.

        Back-propagation must start from a scalar tensor (e.g., a loss).
        """
...
def _squash_gradient(grad: np.ndarray, shape: Sequence[int]) -> np.ndarray:
    """Squashes gradients to account for broadcasting.
...
def _inverse_permute(a: np.ndarray, dims: Sequence[int]) -> np.ndarray:
    ...
    This method is effectively the inverse of `np.transpose()`. That is, if you pass an array with a
    specified set of axes/dimensions to `np.transpose()` and then you call this with the same axes/dimensions,
    you should get back the original array.
