...
class Tensor:
    ...
    This class stores all internal values and gradients as numpy arrays. This
    class includes a simple autograd implementation for supporting back-
    propagation over various operations over tensors.
    ...
    def __init__(
        ...
        Args:
            ...
            requires_grad: Whether gradients should be computed for the tensor
                (e.g., whether the tensor corresponds to weights or constant
                data).
        ...
        if ancestors_requires_grad:
            if requires_grad is not None and not requires_grad:
                raise ValueError(
                    "requires_grad cannot be False if at least one ancestor "
                    "requires gradients"
                )
            if _backward_fn is None:
                raise ValueError(
                    "_backward_fn must be set if at least one ancestor "
                    "requires gradients"
                )
    ...
    def pow(self, other: float | int) -> "Tensor":
        ...
        def _backward_fn(out: "Tensor") -> None:
            ...
            if self.grad is not None:
                self.grad += _unbroadcast_gradient(
                    (other * self.value ** (other - 1)) * out.grad,
                    self.grad.shape,
                )
    ...
    def tensordot(self, other: "Tensor", dims: int = 2) -> "Tensor":
        ...
        def _backward_fn(out: "Tensor") -> None:
            ...
            if self.grad is not None:
                ...
                to_contract_ndim = (
                    out.grad.ndim + other.value.ndim - self.grad.ndim
                ) // 2

                # Gradient update is tensor contraction of output gradient and
                # other's tranpose.
                self.grad += np.tensordot(
                    out.grad,
                    _transpose(other.value, to_contract_ndim),
                    axes=to_contract_ndim,
                )

            if other.grad is not None:
                # Figure out how many dimensions to contract going backwards.
                to_contract_ndim = (
                    out.grad.ndim + self.value.ndim - other.grad.ndim
                ) // 2

                # Gradient update is tensor contraction of self's transpose and
                # output gradient.
                other.grad += np.tensordot(
                    _transpose(self.value, self.value.ndim - to_contract_ndim),
                    out.grad,
                    axes=to_contract_ndim,
                )
    ...
    def __add__(self, other: "Tensor" | ArrayLike) -> "Tensor":
        ...
        def _backward_fn(out: "Tensor") -> None:
            ...
            if _other.grad is not None:
                _other.grad += _unbroadcast_gradient(
                    out.grad, _other.grad.shape
                )
    ...
    def __mul__(self, other: "Tensor" | ArrayLike) -> "Tensor":
        ...
        def _backward_fn(out: "Tensor") -> None:
            if self.grad is not None:
                self.grad += _unbroadcast_gradient(
                    out.grad * _other.value, self.grad.shape
                )
            if _other.grad is not None:
                _other.grad += _unbroadcast_gradient(
                    out.grad * self.value, _other.grad.shape
                )
    ...
    def backward(self) -> None:
        ...
        This method will accumulate gradients for the transitive closure of all
        ancestors in the tensor's computation graph. Back-propagation must
        start from a scalar tensor (typically representing a loss), as all
        gradients are assumed to be computed relative to a scalar loss
        function.

        Only nodes where ``requires_grad`` has been set to True will have
        gradients accumulated into.

        Like the corresponding pytorch method, this method does not
        automatically zero-out gradient buffers. It is the responsibility of
        callers to zero-out gradients (e.g., by setting ``my_tensor.grad =
        None``).
        """
    ...
    def _topological_sort(self) -> list["Tensor"]:
        ...
        The sort order is such that the root node (i.e., self) appears first,
        and all leaf nodes appear later.
        """
        visited_list = []
        visited_set = set()
...
def _unbroadcast_gradient(
    grad: np.ndarray, shape: Sequence[int]
) -> np.ndarray:
    """Squashes gradients in order to account for broadcasting.

    This method sums gradients along dimensions that would be broadcasted
    during some operation. For example, if the target shape is a scalar, then
    this method should sum along all dimensions of the gradient and return
    a scalar.
    ...
    """
    # Prepend extra dimensions of size 1 to shape as necessary to match
    # broadcast rules.
...
def _transpose(a: np.ndarray, dims: int) -> np.ndarray:
    ...
    The last `dims` dimensions of the array are moved to the front of the
    array. For example, if the input array has shape (5, 4, 3) and `dims` is 2,
    then the output array will have shape (4, 3, 5).
...
def _inverse_permute(a: np.ndarray, dims: Sequence[int]) -> np.ndarray:
    ...
    This method is effectively the inverse of `np.transpose()`. That is, if you
    pass an array with `dims` set as the axes to `np.transpose()`, you can
    recover the original array by calling this method on the output.
