...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int) -> None:
        # N.B., initialize all weights/biases using a uniform distribution over
        # [-1, 1].
        self._weights = Tensor(
            np.random.rand(in_features, out_features) * 2 - 1,
            requires_grad=True,
        )
        self._biases = Tensor(
            np.random.rand(out_features) * 2 - 1, requires_grad=True
        )
